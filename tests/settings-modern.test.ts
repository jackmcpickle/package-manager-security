...
function project(name: PackageManager, root = "/p"): Project {
...
function codes(
  ...
  return auditSettings(project(name), policy, { readFile: (p) => files[p] ?? null }).map(
    (f) => f.code,
  );
}
...
function find(
  ...
  return auditSettings(project(name), policy, { readFile: (p) => files[p] ?? null }).find(
    (f) => f.code === code,
  );
}

/** A pnpm project whose only interesting content is the workspace yaml. */
function pnpmFiles(version: string, yaml: string): Record<string, string> {
  return {
...
function yarnFiles(version: string, yaml: string): Record<string, string> {
  return {
...
function bunFiles(install: string): Record<string, string> {
  return {
...
function npmFiles(manifest: string, npmrc: string): Record<string, string> {
  return {
...
test("pnpm 10 still accepts the legacy build allowlist", () => {
  const files = pnpmFiles("10.0.0", "minimumReleaseAge: 10080\nonlyBuiltDependencies:\n  - esbuild\n");
  expect(codes("pnpm", files)).toEqual([]);
});
...
test("relaxed preset skips every release-age check", () => {
  ...
  expect(codes("pnpm", files, relaxed).filter((c) => c.startsWith("min-age"))).toEqual([]);
});
...
test("a wildcard minimumReleaseAgeExcludes voids the bun gate", () => {
  const files = bunFiles('minimumReleaseAge = 604800\nminimumReleaseAgeExcludes = ["*"]\n');
...
test("allowScripts without strict-allow-scripts is only advisory", () => {
  const files = npmFiles(`{"name":"x","allowScripts":{"esbuild@0.2.5":true}}`, "");
...
test("ignore-scripts masking an allowScripts policy is reported", () => {
  const files = npmFiles(`{"name":"x","allowScripts":{"esbuild@0.2.5":true}}`, "ignore-scripts=true\n");
  const found = codes("npm", files);
...
function uvFiles(toolUv: string): Record<string, string> {
  return {
...
test("a wildcard exclude-newer-package voids the uv gate", () => {
  const files = uvFiles(`exclude-newer = "7 days"\n\n[tool.uv.exclude-newer-package]\n"*" = false\n`);
...
test("apply migrates the pnpm legacy build allowlist into allowBuilds", () => {
  ...
  const findings = auditSettings(target, loadPolicy({}), { readFile: (p) => files[p] ?? null });
