...
**Goal:** Ship `pmsec`, a local CLI that audits package-manager security settings and advisories across monorepos and folders of many projects, and applies fixes only when explicitly asked.

**Architecture:** Bun + TypeScript library with a thin CLI. File-based settings checks need no PM binary. Advisories and migrate shell out through injected runners. Policy is layered TOML. Discovery is hybrid (git repos → workspace/PM roots). Cache lives in `~/.cache/pmsec/` (overridable). Apply is serial; audit concurrency defaults to 4.
...
- Config files: user `~/.config/pmsec/config.toml` (or `$XDG_CONFIG_HOME/pmsec/config.toml`); scan-root and per-repo `.pmsec.toml`. Closer wins. Flags win over files. Per-PM tables: `[npm]`, `[pnpm]`, `[yarn]`, `[bun]`, `[uv]`. Never execute JS/TS config.
- Cache dir: `~/.cache/pmsec/` (or `$XDG_CACHE_HOME/pmsec/`). Lockfile-digest entries + shared package@version. Do not write into `uv cache` or the pnpm store. `--refresh` / `--no-cache` bypass. Settings checks never use the advisory cache.
...
```json
{
  ...
  "version": "0.1.0",
  "type": "module",
  "bin": { "pmsec": "./src/main.ts" },
  "scripts": {
    "test": "bun test",
    "build": "bun build ./src/main.ts --compile --outfile dist/pmsec"
  }
}
```

`src/cli.ts` — `run([])` returns `{ exitCode: 2 }` and writes `Usage: pmsec <command>` to stderr. Do not implement `audit` yet.
...
git commit -m "feat: scaffold pmsec CLI with usage exit"
```
...
git commit -m "feat: load layered pmsec policy from TOML and flags"
```
...
```
pmsec audit [path] [--preset standard] [--apply] [--apply-advisories] [-i] [--concurrency 4] [--json] [--sarif] [--report file.md] [--force] [--commit] [--refresh] [--no-cache] [--allow-majors]
```

This task implements `pmsec audit [path]` **settings + preflight only** (ignore apply flags: if `--apply` is passed, print to stderr `apply is not implemented` and still run audit / exit 2). Default path: cwd.

Policy files: read `XDG_CONFIG_HOME/pmsec/config.toml` or `HOME/.config/pmsec/config.toml`, then `<path>/.pmsec.toml`, then each project `.pmsec.toml`.
...
git commit -m "feat: pmsec audit reports settings findings and exit codes"
```
...
test("identical lockfile digest within TTL skips the live runner", async () => {
  const calls: string[][] = [];
  const cache = createFsCache("/tmp/pmsec-test-cache", () => 1_000, 86_400_000);
...
test("package@version cache hit still runs live audit", async () => {
  const calls: string[][] = [];
  const cache = createFsCache("/tmp/pmsec-test-cache2", () => 1_000, 86_400_000);
...
Use a temp dir in tests; do not touch the real `~/.cache/pmsec` from tests.
...
GitHub Action workflow presets (`standard` / `strict` / `relaxed`) that run `pmsec audit` and fail on exit 1 (and optionally 2).
