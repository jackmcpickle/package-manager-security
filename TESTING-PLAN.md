# pmguard Testing Plan

> Audit of the current suite (2026-08-20, `feat/pmguard`, 220 tests / 13 files). Waves 1–4 are done. Tests stay at the eight seams in `PLAN.md`. Do not mock our own modules. Inject filesystem, PATH, runners, time, and cache dir.
...
| **E2E / smoke** | Compiled `dist/pmguard audit` on `tests/fixtures/discover/many-repos` | later, one smoke | 1 test |
...
| P6 | CLI `--preset` beats repo `.pmguard.toml` | integration | done |
...
| P9 | `XDG_CONFIG_HOME` wins over `~/.config/pmguard` when CLI loads files | integration | done |
...
Migrate-to-uv is not implemented: interactive poetry does not offer migrate and still uses OSV. Compiled-binary smoke: `bun run build:binary && ./dist/pmguard audit tests/fixtures/discover/many-repos --json`.
